...
const ProjectOverview: React.FC = () => {
  ...
  const [projectAlerts, setProjectAlerts] = useState<ProjectAlertItem[]>([]);
  ...
  useEffect(() => {
    ...
    const repo = getRepo(location.pathname, companyId);
    Promise.all([
      repo.listProjects(),
      listActivityLogs({ orgId: companyId, limit: 12 }),
    ])
      .then(([data, logs]) => {
        if (isActive) {
          ...
          setProjectAlerts(
            logs.slice(0, 6).map((log) => ({
              id: log.id,
              projectId: id,
              type: log.status === 'fail' ? 'error' : log.status === 'warn' ? 'warning' : 'info',
              title: log.module,
